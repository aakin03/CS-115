...
def questifyAlt(str_list):
    ...
    if str_list == []:
        return []
    return map(lambda n: n + '?', str_list)
...
def letterScoreLoop(letter, scorelist):
    ...
    while n <= len(scorelist):
        if letter == scorelist[n][0]:
            return scorelist[n][1]
        n = n + 1
...
def wordScoreLoop(S, scorelist):
    ...
    for x in range(len(S)):
        total = total + letterScoreLoop(S[x], scorelist)
    return total
...
def wordsWithScoreLoop(dct, scores):
    ...
    for x in range(len(dct)):
        new += [[dct[x], wordScoreLoop(dct[x], scores)]]
    return new